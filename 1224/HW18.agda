...
foldr-map-fusion f _⊕_ e xs =
  ...
  ≡⟨ cong (foldr _⊕_ e) (map-is-foldr xs) ⟩
    (foldr _⊕_ e ∘ foldr (λ z → _∷_ (f z)) []) xs
  ≡⟨ foldr-fusion (foldr _⊕_ e) [] (λ x y → refl) xs ⟩
...
map-composition f g xs = 
  begin
    (map f ∘ map g) xs
  ≡⟨ map-is-foldr (map g xs) ⟩
    (foldr (λ z → _∷_ (f z)) [] ∘ map g) xs
  ≡⟨ foldr-map-fusion g (λ z → _∷_ (f z)) [] xs ⟩
